...
class MultiQuadrotorCSPointmass(BaseModel):
    ...
    def __init__(self, **kwargs):
        ...
        self._mjMdl = MujocoModel(model_path="multi_quad_pointmass.xml",
        ...
        self._inertia_matrix = np.array(
            [[0.0023, 0.0, 0.0], [0.0, 0.0023, 0.0],
             [0.0, 0.0,
              0.004]])  # TODO using same inertia matrix for all quadrotors
    ...
    def _query_latest_state(self):
        for i in range(self.nQ):
            self._state.quads[i].position = self.mjMdl.data.qpos[7 * i:7 * i +
                                                                 3]
            ...
            self._state.quads[i].velocity = self.mjMdl.data.qvel[6 * i:6 * i +
                                                                 3]
            self._state.quads[i].angvel = self.mjMdl.data.qvel[6 * i +
                                                               3:6 * i + 6]

        self._state.load_position = self.mjMdl.data.qpos[7 *
                                                         self.nQ:7 * self.nQ +
                                                         3]
        self._state.load_velocity = self.mjMdl.data.qvel[6 *
                                                         self.nQ:6 * self.nQ +
                                                         3]
        ...
        for i in range(self.nQ):
            ...
            self._state.cables[i].dq = (self._state.load_velocity -
                                        self._state.quads[i].velocity)
            self._state.cables[i].omega = np.cross(self._state.cables[i].q,
    ...
    def quad_position_control(self):
        ...
        for i in range(self.nQ):
            ...
            ex = self._state.quads[i].position - self._init_state.quads[
                i].position
