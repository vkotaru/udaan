...
class QuadrotorCSPayload(base.QuadrotorCSPayload):
    ...
    def __reset_tendon_model(self):
        quat_Q = np.array([1., 0., 0., 0.])  # quadrotor attitude (w, x, y, z)
        ...
        self._mjMdl.data.qvel[self.
                              _mj_quad_omega_index:self._mj_quad_omega_index +
                              3] = self.state.angular_velocity  # quadrotor angular velocity

        self._mjMdl.data.qpos[self._mj_payload_pos_index:self.
                              _mj_payload_pos_index + 3] = self.state.payload_position
        self._mjMdl.data.qpos[self._mj_payload_quat_index:self.
                              _mj_payload_quat_index + 4] = np.array(
                                  [1., 0., 0.,
                                   0.])  # point mass payload, this is obsolete
        self._mjMdl.data.qvel[self._mj_payload_vel_index:self.
                              _mj_payload_vel_index +
