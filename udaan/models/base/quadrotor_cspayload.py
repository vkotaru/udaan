...
class QuadrotorCSPayload(BaseModel):
    ...
    def _zoh(self, thrust: float, torque : np.ndarray):
        ...
        payload_accel = -self._ge3 + ((np.dot(q, fRe3)
                                - self.mQ*self.l*np.dot(dq, dq))/(self.mQ+self.mL))*q
        ...
        self.state.orientation = self.state.orientation @ expm(utils.hat(self.state.angular_velocity*h))
        ang_vel_dot = self.JQinv @ (torque - np.cross(self.state.angular_velocity, self.JQ @ self.state.angular_velocity))
    ...
    def _parse_input(self, input):
        if self._input_type == QuadrotorCSPayload.INPUT_TYPE.CMD_ACCEL:
            thrust, torque  = self._att_controller.compute(self.t, (self.state.orientation, \
              self.state.angular_velocity), input)
    ...
    def update_render(self):
        if self.render:
            self._vfx.update(self.state.payload_position, self.state.cable_attitude, self.state.orientation)
        return
    ...
    def reset(self, **kwargs):
        ...
        k = ["position", "velocity", "orientation", "angular_velocity", "payload_position", \
          "payload_velocity", "cable_attitude", "cable_ang_velocity"]
        for key in k:
            if key in kwargs:
