...
class QuadrotorCSPayload(BaseModel):
    ...
    def _zoh(self, thrust: float, torque: np.ndarray):
        ...
        self.state.cable_attitude = expm(
            utils.hat(h * self.state.cable_ang_velocity)) @ q
    ...
    def update_render(self):
        if self.render:
            self._vfx.update(self.state.payload_position,
                             self.state.cable_attitude, self.state.orientation)
        return
    ...
    def reset(self, **kwargs):
        ...
        if "position" in kwargs:
            self.state.payload_position = kwargs[
                "position"] + self.l * self.state.cable_attitude
        # if only payload position is given, set position accordingly
        if "payload_position" in kwargs:
            self.state.position = kwargs[
                "payload_position"] - self.l * self.state.cable_attitude
