...
class QuadCSPayloadController(PDController):
    def __init__(self, **kwargs):
        ...
        self._gain_pos = Gains(
            kp=np.array([4.0, 4.0, 8.0]), kd=np.array([3.0, 3.0, 6.0])
        )
        self._gain_cable = Gains(
            kp=np.array([24.0, 24.0, 24.0]), kd=np.array([8.0, 8.0, 8.0])
        )
    ...
    def compute(self, *args):
        ...
        # payload position control
        Fff = (self.mQ + self.mL) * (d2sd + self._ge3) + self.mQ * self.l * np.dot(
            dq, dq
        ) * q
