...
def multi_quad_pointmass(nQ=2, filename="./assets/multi_quad_pointmass.xml"):
    cable_length = 1.0
    z = 2.0
    r = 0.5
    ...
    mjcWriter.site(
        pyld, "end2", pos=np.array([0.0, 0.0, 0.0]), type="sphere", size=[0.01]
    )
    ...
    for i in range(nQ):
        mjcWriter.spatial(
            tendon, "quad" + str(i) + "_end1", "end2", range=[0.0, cable_length]
        )

    mjcWriter.save_to(filename, verbose=True)
    return
...
def create_quadcopter():
    ...
    mjcWriter.create_quadrotor(
        mjcWriter.worldbody, "quadrotor", np.array([0.0, 0.0, 1.0])
    )
...
def quadrotor_payload():
    ...
    quad = mjcWriter.create_quadrotor0(
        mjcWriter.worldbody, "quadrotor", np.array([0.0, 0.0, 2.0])
    )
    sitef = mjcWriter.site(
        quad, "thrust", pos=np.array([0.0, 0.0, 0.0]), rgba=[0.0, 1, 1, 1.0]
    )
