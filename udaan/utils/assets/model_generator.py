...
def multi_quad_pointmass(nQ=2, filename="./assets/multi_quad_pointmass.xml"):
    ...
    for i in range(nQ):
        q1 = mjcWriter.create_quadrotor0(mjcWriter.worldbody, "quad"+str(i), np.array([r*np.cos(TH[i]), r*np.sin(TH[i]), z]))
    
    pyld = mjcWriter.body(mjcWriter.worldbody, "pyld", pos=np.array([0., 0., zL]))
    mjcWriter.sphere(pyld, "pyld", radius=0.05, rgba=[0.0, 1, 1, 1.0], mass=0.15)
    mjcWriter.site(pyld, "end2", pos=np.array([0., 0., 0.]), type="sphere", size=[0.01])
    mjcWriter.joint(pyld, "pyld_joint", type="free")
    
    tendon = mjcWriter.tendon(mjcWriter.root)
    for i in range(nQ):
        mjcWriter.spatial(tendon, "quad"+str(i)+"_end1", "end2", range=[0., cable_length])
